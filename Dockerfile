...
ARG AUTH0_PROVIDER_VERSION="v0.2.0"
...
RUN apk add --no-cache ca-certificates bash curl git \
    ...
    && rm -rf terraform.zip \
    && wget -q -O terraform-provider-auth0.tar.gz https://github.com/alexkappa/terraform-provider-auth0/releases/download/${AUTH0_PROVIDER_VERSION}/terraform-provider-auth0_${AUTH0_PROVIDER_VERSION}_linux_amd64.tar.gz \
    && tar xvzf terraform-provider-auth0.tar.gz -C /usr/local/bin \
